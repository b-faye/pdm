...
def get_dataloaders(dataset_name='flwrlabs/celeba', batch_size=64, num_workers=2):
    # Load dataset
    ...
    selected_indices = []
    for label in sorted(class_counts.keys())[:2]:
        indices = class_counts[label]
        if len(indices) >= 25000:
            selected_indices.extend(random.sample(indices, 10000))
        else:
            raise ValueError(f"Class {label} has less than 10000 images.")
...
def generate_and_save_images(config, pipeline, accelerator, total_images=20000):
...
def save_real_images(config, real_dataloader, total_images = 20000):
